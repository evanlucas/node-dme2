/*!
 *	Filename: secondarydns.js
 *	Author: Evan Lucas
 *	Copyright: 2012-2013 5060
 *	Description: SecondaryDNS module for DNSMadeEasy API V2.0
 *
 ...
 */
var SecondaryDNS = exports.SecondaryDNS = function(client) {
	if (!client) throw new Error('SecondaryDNS must be constructed with a client');
...
SecondaryDNS.prototype = {
	...
	getAllDomains: function() {
		...
		var self = this;
		self.client.get('dns', 'secondary', function(err, data) {
	...
	updateAllDomains: function() {
	    ...
	    var self = this;
	    self.client.put('dns', 'secondary', putData, function(err, data) {
		    if (err) return cb(err);
		    cb(null, data);
	    });
	},
	...
	getDomain: function() {
		...
		var self = this;
		self.client.get('dns', 'secondary', domain_id, function(err, data) {
			if (err) return cb(err);
			cb(null, data);
		});
	},
	...
	deleteDomain: function() {
		...
		var self = this;
		self.client.delete('dns', 'secondary', domain_id, function(err, data){
			if (err) return cb(err);
			cb(null, data);
		});
	},
	...
	updateDomain: function() {
	    ...
	    if (!domain_id) return cb({status: 'error', message: 'Invalid domain id given'});
	    var self = this;
	    self.client.put('dns', 'secondary', domain_id, putData, function(err, data) {
		    if (err) return cb(err);
		    cb(null, data);
	    });
	},
	...
	getRecordsForDomain: function() {
		...
		if (!domain_id) return cb({status: 'error', message: 'Invalid domain id given'});
		var self = this;
		self.client.get('dns', 'secondary', domain_id, 'records', function(err, data) {
			if (err) return cb(err);
			cb(null, data);
		});
	},
	...
	deleteRecordsForDomain: function() {
		...
		if (!domain_id) return cb({status: 'error', message: 'Invalid domain id given'});
		var self = this;
		self.client.delete('dns', 'secondary', domain_id, 'records', function(err, data){
			if (err) return cb(err);
			cb(null, data);
		});
	},
	...
	createRecordsForDomain: function() {
	    ...
	    if (!domain_id) return cb({status: 'error', message: 'Invalid domain id given'});
	    var self = this;
	    self.client.post('dns', 'secondary', domain_id, postData, function(err, data) {
		    if (err) return cb(err);
		    cb(null, data);
	    });
	}
